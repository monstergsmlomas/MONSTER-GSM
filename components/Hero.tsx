...
export default function Hero() {
  ...
  // GSAP entrance
  useGSAP(() => {
    gsap.set([logoWrapRef.current, badgeRef.current, headingRef.current,
              dividerRef.current, taglineRef.current, pillarsRef.current,
              brandsRef.current, ctaRef.current, scrollIndRef.current],
      { autoAlpha: 0 });
    gsap.set(logoWrapRef.current, { scale: 0.1, rotation: -20 });
    gsap.set([badgeRef.current, taglineRef.current, pillarsRef.current, brandsRef.current], { y: 22 });
    ...
    tl
      .to(logoWrapRef.current, { scale: 1.08, autoAlpha: 1, rotation: 3, duration: 0.5, ease: 'power4.out' })
      .to(logoWrapRef.current, { scale: 1, rotation: 0, duration: 0.5, ease: 'elastic.out(1.2, 0.5)' })
      .to(badgeRef.current,    { y: 0, autoAlpha: 1, duration: 0.4 }, '-=0.2')
      .to(headingRef.current,  { y: 0, autoAlpha: 1, duration: 0.55 }, '-=0.15')
      .to(dividerRef.current,  { scaleX: 1, duration: 0.5, ease: 'power2.inOut' }, '-=0.2')
      .to(taglineRef.current,  { y: 0, autoAlpha: 1, duration: 0.45 }, '-=0.2')
      .to(pillarsRef.current,  { y: 0, autoAlpha: 1, duration: 0.45 }, '-=0.3')
      .to(brandsRef.current,   { y: 0, autoAlpha: 1, duration: 0.4 }, '-=0.2')
      .to(ctaRef.current,      { y: 0, autoAlpha: 1, duration: 0.5, ease: 'back.out(1.5)' }, '-=0.15')
      .to(scrollIndRef.current,{ autoAlpha: 1, duration: 0.4 }, '-=0.05');
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-4 max-w-4xl mx-auto">
        ...
        {/* Logo */}
        <div ref={logoWrapRef} className="mb-7"
        ...
        {/* Badge */}
        <div ref={badgeRef} className="mb-5 inline-flex items-center gap-2 px-4 py-1.5 rounded-full"
        ...
        {/* Brand name — Betterlett, solo M mayúscula */}
        <h1 ref={headingRef} className="mb-2 leading-none"
          style={{ fontFamily: 'Betterlett, sans-serif', fontSize: 'clamp(3.5rem, 12vw, 7rem' }}>
          <span style={{ color: '#3DFF14' }}>Monster</span>
          <span style={{ color: '#1A5CFF' }}> gsm</span>
        </h1>

        <div ref={dividerRef} className="w-72 section-line my-5" />

        <p ref={taglineRef} className="font-rajdhani text-xl sm:text-2xl font-bold text-white mb-6 tracking-wide max-w-2xl">
          Reparamos tu celular —{' '}
          <span style={{ color: '#3DFF14' }}>iPhone</span>
          {' '}&amp;{' '}
          <span style={{ color: '#6a9fff' }}>Android</span>
        </p>

        <div ref={pillarsRef} className="flex flex-wrap justify-center gap-4 mb-6">
          {pillars.map(({ icon: Icon, text }, i) => (
            <div key={i} className="flex items-center gap-2 px-4 py-2 rounded-lg font-rajdhani text-sm font-semibold text-gray-300"
        ...
        <div ref={brandsRef} className="mb-10 flex flex-wrap justify-center gap-x-5 gap-y-1">
          {brands.map((b, i) => (
            <span key={i} className="font-rajdhani text-sm font-semibold tracking-wider"
      ...
      <a ref={scrollIndRef} href="#servicios"
        className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-1"
        style={{ color: 'rgba(255,255,255,0.25)' }} aria-label="Scroll down">
